...
def test(s, ss):
    ...
    high1 = high_enha + high_spec + high_stru + high_wate + ['校', '梁', '柱', '叶', '翼', '粱', '焊', '切', '加强件'] + ["纵梁",
                                                                                                               "车顶",
                                                                                                               "避震器",
                                                                                                               "防火墙",
                                                                                                               "A柱",
    ...
    #          '后围']
    rep = ['翼子板(喷漆)', '钣金:客户付款;', '前叶', '前翼', '发动机机油', '后叶子板整形', '后叶子板修复', '后叶子板喷漆', '发动机盖', '发动机故障灯', '排水软管', '发动机润滑',
           '后叶整形', '后叶喷漆', '发动机油',
           '后翼子板(喷漆)', '下裙边喷漆', '叶子板烤漆', '立柱饰板', '发动机冷却液', '槛喷漆', '发动机罩', '发动机漏油', '后叶修复', '后叶补漆', '发动机线路', '后围整形',
           '后围油漆', '发动机皮带',
           '钣金:客户付款;', '发动机舱', '发动机右下护板', '后围板修复', '后围板喷漆', '减震器总成', '减振器总成','A柱喷漆','B柱喷漆','C柱喷漆']
...
def test2(s, ss):
    ...
    key_word2 = ["纵梁", "车顶", "避震器", "防火墙", "A柱", "B柱", "C柱", "气囊", "备胎室", "泡水", "火烧", "水泡", "翼子板", "后叶", '叶子板', '前柱',
                 '后柱', '梁头', '气帘', '焊', '切', '大梁', '加强件', '后侧围件', '中立柱', 'D柱', '校', '减震器', '梁', '柱', '叶', '翼', '粱',
                 '减振器', '后围'
        , '切割', '翅', '车顶', '全损', '事故', '上边梁', '气囊', '纵', '拆装', '火烧', '水泡', '后围', '后围板', '侧围', '侧围板', '左侧围', '右侧围',
                 '后侧围', '后翼子板'
...
def shigu_filter(data):
    li = ["后叶拆装", '修复A柱', '修复B柱', '修复C柱', 'B柱钣金', 'C柱钣金', 'A柱钣金', 'A柱修复', 'B柱修复', 'C柱修复', ':气囊:', ';气囊;', ';安全气囊;',
          ...
          '修复左右大梁','后叶子板切割','后围切割','中央立柱(左侧):修理','中央立柱(右侧):修理','后围板:更换','大梁更换','更换左右后叶','左后翼子板：更换','右后翼子板：更换',
          '更换：右后叶子板','更换：左后叶子板','更换主副气囊','更换大顶;','更换气囊;']
    li += ['车辆涉水', '水淹车', '水浸车', '水淹事故', '水淹', '涉水车', '泡水车', '进水车', '车辆泡水', 'A柱(右侧):修理', 'A柱(左侧):修理', 'B柱(右侧):修理',
           'B柱(左侧):修理','C柱(右侧):修理', 'C柱(左侧):修理']
    li += ['全损车']
...
def baoyang_filter(data):
    ...
    for x in data:
        for y in x['records']:
            if y['label'] == 9 and y['detail'] != None and type_list[0] in y['detail'] and len(y['detail']) + len(
                    y['other']) < 100:
                y['label'] = 0
...
def not_in_filter(data):
    for x in data:
        for y in x['records']:
            if y['detail'] != None and not test(y['detail'], y['other']) and y['label'] == 9 and len(y['detail']) + len(
                    str(y['other'])) < 70:
                y['label'] = 0
